...
% Um also das Suffix-Array aus $V$ berechnen zu können, setzen wir lediglich nach Terminierung des Algorithmus $SA[V[i]]=i$ und erhalten dadurch unser Ergebnisarray, im Folgendem anhand des Beispiels aus 3.3 gezeigt:\\
%\begin{center}
...
%Wir definieren uns hierfür einen Zahlenbereich [$l,k$) wobei $k-l$= $\sigma$, beispielsweise [1,$\sigma$). Initial transformieren wir den String $X$ in das Array $V$ mit $V[i]=x_i - l +1$ für $i \in [0,n)$. Für das Sentinel legen wir in $V[n]$ dann eine Null an. Sei zudem $r$ die größtmögliche Ganzzahl, sodass $(\sigma+1)^r-1$ in ein Maschinenwort passt, also im Fall von 64-Bit Maschinen $\leq 2^{63}$. Aufbauend darauf, definieren wir die Einträge in $V$ erneut durch:
%\begin{equation}
