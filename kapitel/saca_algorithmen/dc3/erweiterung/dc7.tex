...
Nun werden Tupel der Länge $l+1$ aufgestellt, die aus $l$ Zeichen beginnend von $i$ beziehungsweise $j$ und dem Rang der Position $i+l$ beziehungsweise $j+l$ bestehen, also $(T[i, i+l], Rang[i+l+1])$ und $(T[j, j+l], Rang[j+l+1])$. Wird dieser Ansatz verwendet, muss ein $\Theta(7)$-Wege-Merge mit jeweils $\mathcal{O}(7)$-Vergleichen durchgeführt werden, was zu einer \emph{Worst-Case}-Laufzeit von $\mathcal{O}(7n \text{ log} (7))$ führt.
...
Interessant ist, ob der $\emph{DC7}$ gegenüber dem $\emph{DC3}$ Vorteile oder sogar auch Nach\-tei\-le aufweist. In der Theorie hat der \emph{Difference Cover} - Algorithmus eine Laufzeit von $\mathcal{O}(vn)$ und somit ist die Laufzeit des $\emph{DC3}$ - Algorithmus zwar besser also die des $\emph{DC7}$ - Algorithmus, aber der String wird bei $\emph{DC3}$ - Algorithmus in jedem Rekursionsaufruf nur um $T(2n/3)$ verkleinert. Bei dem $\emph{DC7}$ - Algorithmus wird der String in der Rekursion um $T(3n/7)$ verkleinert. Wie dies in der Praxis aussieht, wird in dem Kapitel \ref{dc3:optim} näher untersucht.