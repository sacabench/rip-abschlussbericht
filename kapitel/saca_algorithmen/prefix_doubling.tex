...
Wir betrachten zunächst in Abschnitt \ref{algo:doubling:sec:doubling} den Doubling-Algorithmus \cite{Arge1997}\cite{Crauser2002}. Dieser hat eine I/O-Komplexität von $\mathcal{O}(\text{sort}(n \log \text{maxlcp}))$ und erlaubt es somit Strings der Länge $2^k$ in $k$ Iterationen zu sortieren.
...
Wir betrachten nun wieder die I/O-Komplexität. Beim naiven Doubling-Algorithmus sortieren und verdoppeln wir so lange alle Präfixe, bis sie paarweise verschieden sind. Jeder einzelne Präfix durchläuft somit immer genau $\log \text{maxlcp}$ Iterationen. Mit Discarding hingegen wird ein einzigartiger Präfix so früh wie möglich aus der Iteration entfernt. Somit gilt für alle $\suffix i$, dass ein Präfix maximal $\text{dps}(i)$ Iterationen durchläuft.
...
\item Unmittelbar nach dem Sortierschritt findet entweder eine Paarbildungs- oder eine Discarding-Phase statt, die sequentiell alle sortierten $(c, i)$ Tupel verarbeitet.
