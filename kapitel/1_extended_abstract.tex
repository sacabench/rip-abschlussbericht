...
You can evaluate a single algorithm with the terminal command \termfont{sacabench} \termfont{construct} followed by the desired SACA and a path to an input file.
...
\termfont{sacabench/sacabench construct BPR /path/to/input -c -p 1K -r 2\\ -{}-rplot -{}-latexplot  -b /destination/path/to/result.json -f}
...
\termfont{sacabench/sacabench construct BPR /path/to/input -{}-config\\ /path/to/config }
...
The GPU SACAs are run on an NVIDIA\textregistered Tesla K40.
...
\begin{figure}[!ht]
    ...
    \caption{Comparison of the in \sacabench included parallel implementations of SACAs, including time and memory consumption: wiki.txt}
...
\begin{figure}[!ht]
    ...
    \caption{Comparison of the in \sacabench included parallel implementations of SACAs, including time and memory consumption: dna.txt}
...
\begin{figure}[!ht]
    ...
    \caption{Comparison of the in \sacabench included parallel implementations of SACAs, including time and memory consumption: commoncrawl.txt}
...
The parallel DivSufSort uses little extra memory and still has great performance until 8 cores (1600 MiB text).
After this point, the text size (3200 MiB) is too large to be addressed with 32-bits
and since the implementation we used supports only 32-bit suffix indices, the suffix array can't be constructed correctly.
This is also the case for the lines for the other algorithms that stop at 8 cores.

Deep-Shallow stops at 800 MiB, because it produces Segfaults from 1600 MiB on.
Sadly we couldn't fix this problem in time.
...
\end{figure}
\FloatBarrier
Figures \ref{ea:gpu_benchmark:1} and \ref{ea:gpu_benchmark:2} show the runtime comparisons between the parallel version of Mori's DivSufSort and the variants of the Osipov prefix doubler implemented by us, both the parallel variant for the CPU and for the GPU. Both CPU implementations use the 20 available cores.
