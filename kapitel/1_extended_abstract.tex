...
\begin{figure}[!h]
    ...
    \caption{Example of runtime and memory consumption of the phases of DivSufSort, measured by \sacabench and visualized on the tudostat website.}
    \label{ea:phases}
\end{figure}
...
This file can be converted to a plot on the tudostat website (see Figure \ref{ea:phases} as an example).
...
It influenced divsufsort, which sorts its RMS (rightmost S-Type) substrings before it completely sorts all RMS suffixes. Given the correct order of the RMS suffixes, all remaining S-Type suffixes get induced in one pass and all L-Type suffixes in a second pass.
...
We show that this algorithm outperformes the parallel DivSufSort implementation, although the required and available memory on current GPUs still restricts the algorithm to far smaller input sizes than CPU SACAs are capable of. Further research of GPU SACAs should be considered, as we could hardly find any (working) reference algorithms to compare our implementation with (and development of GPUs shows higher performance improvements than CPU development\todo{entweder belegen oder entfernen.}).
