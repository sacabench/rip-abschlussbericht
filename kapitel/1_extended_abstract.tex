...
However, caused by source code incompatibilities, memory utilization for BPR is not intuitively comparable to other algorithms as we are only able measure absolute memory but not extra memory for this specific algorithm. The runtime performance and memory consumption of the DC3 implementation provided in paper \cite{saca:9} is bad, because it is not optimized for demonstration. The authors wanted to write the implementation in only $50$ lines of code.
