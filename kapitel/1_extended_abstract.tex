...
Since GPUs feature many more cores than modern desktop or even server CPUs,
we include a prefix doubling SACA which uses CUDA to run on the GPU~\cite{osipovGPU}.
We show that this algorithm outperformes the parallel DivSufSort implementation.
\todo{Olli? Hermann? vlt noch was ergänzen}
...
Evaluation of both the sequential SACAs, including the reference algorithms, as well as of the parallel SACAs is done on LiDo3 \cite{lido3}. The system's specifications are described in table \ref{ea:lido}.
...
The texts are \texttt{wiki.txt} (a current XML-dump of Wikipedia),
\texttt{dna.txt} (a cleaned-up version from the 1000 Genome Project~\cite{1000genomeproject})
and \texttt{commoncrawl.txt} (an ASCII-only subset of the Commoncrawl dataset~\cite{commoncrawl}).
